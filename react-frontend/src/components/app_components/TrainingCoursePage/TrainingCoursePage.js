...
import TrainingCourseDatatable from "./TrainingCourseDataTable";
import TrainingCourseEditDialogComponent from "./TrainingCourseEditDialogComponent";
import TrainingCourseCreateDialogComponent from "./TrainingCourseCreateDialogComponent";
import TrainingCourseFakerDialogComponent from "./TrainingCourseFakerDialogComponent";
import TrainingCourseSeederDialogComponent from "./TrainingCourseSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TrainingCoursePage = (props) => {
    ...
    const filename = "trainingCourse.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("trainingCourse");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , material : urlParams.singleTrainingMaterialId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "TrainingCourse", type: "error", message: error.message || "Failed get TrainingCourse" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("trainingCourse").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "TrainingCourse", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("trainingCourse").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/trainingCourse/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="trainingCourse-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="trainingCourse-datatable">
                    <TrainingCourseDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TrainingCourseEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TrainingCourseCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TrainingCourseFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TrainingCourseSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TrainingCoursePage);
