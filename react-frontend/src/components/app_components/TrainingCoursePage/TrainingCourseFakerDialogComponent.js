...
import trainingCourseFakerFactory from "./trainingCourseFakerFactory";
...
const TrainingCourseFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("trainingMaterial")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setmaterialIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = trainingCourseFakerFactory(props.user,fakerCount,materialIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingCourse").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingCourseFakerDialogComponent);
