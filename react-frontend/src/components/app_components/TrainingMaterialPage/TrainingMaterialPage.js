...
import TrainingMaterialDatatable from "./TrainingMaterialDataTable";
import TrainingMaterialEditDialogComponent from "./TrainingMaterialEditDialogComponent";
import TrainingMaterialCreateDialogComponent from "./TrainingMaterialCreateDialogComponent";
import TrainingMaterialFakerDialogComponent from "./TrainingMaterialFakerDialogComponent";
import TrainingMaterialSeederDialogComponent from "./TrainingMaterialSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TrainingMaterialPage = (props) => {
    ...
    const filename = "trainingMaterial.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("trainingMaterial");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Training Material", type: "error", message: error.message || "Failed get Training Material" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("trainingMaterial").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Training Material", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("trainingMaterial").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/trainingMaterial/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="trainingMaterial-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="trainingMaterial-datatable">
                    <TrainingMaterialDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TrainingMaterialEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TrainingMaterialCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TrainingMaterialFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TrainingMaterialSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TrainingMaterialPage);
