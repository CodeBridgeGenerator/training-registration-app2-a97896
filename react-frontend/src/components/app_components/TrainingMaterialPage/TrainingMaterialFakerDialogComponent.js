...
import trainingMaterialFakerFactory from "./trainingMaterialFakerFactory";
...
const TrainingMaterialFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trainingMaterialFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingMaterial").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingMaterialFakerDialogComponent);
